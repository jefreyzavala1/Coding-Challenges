...
He started with summing and did it easily, but decided to store the sum he found in some random position of the original array which was a bad idea.
Now he needs to cope with the second task, sorting the original array arr, and it's giving him trouble since he modified it.
...
function shuffledArray(shuffled) {
    ...
    let sumOfAll = shuffled.reduce((accu,current)=> accu + current,0);
    
    while(shuffled.length!= 0){
      if(shuffled[0] === (sumOfAll - shuffled[0])){
        shuffled.shift();
        break;
      }else{
        let item = shuffled[0];
